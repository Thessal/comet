// use parser::behavior::{BehaviorDecl, Expr, Ident, Literal, TypeDecl};
...
//             }
//         }
//         Expr::List(exprs) => Expr::List(exprs.iter().map(|e| substitute_expr(e, env)).collect()),
//         Expr::Range { start, step, end } => Expr::Range {
...
//         },
//         Expr::Call { path, args } => {
//             let new_args = args.iter().map(|arg| substitute_expr(arg, env)).collect();
//             Expr::Call {
//                 path: path.clone(),
//                 args: new_args,
//             }
//         }
//         Expr::MemberAccess { target, field } => Expr::MemberAccess {
...
//         expr: &Expr,
//         behaviors: &HashMap<String, BehaviorDecl>,
//         library: &[FnSignature],
//     ) -> Result<Vec<Vec<RealExpr>>, String> {
//         match expr {
...
//                     Self::get_lit(st)?
//                 } else {
//                     if let Literal::Integer(_) = start_lit {
//                         Literal::Integer(1)
//                     } else {
//                         Literal::Float(1.0)
//                     }
