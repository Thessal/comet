...
impl AlphaConvEncoder {
    ...
    pub fn forward(&self, alpha_matrix: &Tensor) -> Tensor {
        ...
        let x = x.adaptive_max_pool1d(&[1]).0;
...
impl TransformerDecoderLayer {
    ...
    pub fn forward(&self, tgt: &Tensor, memory: &Tensor, tgt_mask: &Tensor) -> Tensor {
        ...
        // 1. Self Attention
        let q = self.self_attn_q.forward(tgt).view([b, s, self.num_heads, self.head_dim]).transpose(1, 2);
        let k = self.self_attn_k.forward(tgt).view([b, s, self.num_heads, self.head_dim]).transpose(1, 2);
        let v = self.self_attn_v.forward(tgt).view([b, s, self.num_heads, self.head_dim]).transpose(1, 2);
        ...
        let context = attn_weights.matmul(&v).transpose(1, 2).contiguous().view([b, s, e]);
        ...
        let q_cross = self.cross_attn_q.forward(&tgt).view([b, s, self.num_heads, self.head_dim]).transpose(1, 2);
        let k_cross = self.cross_attn_k.forward(memory).view([b, mem_s, self.num_heads, self.head_dim]).transpose(1, 2);
        let v_cross = self.cross_attn_v.forward(memory).view([b, mem_s, self.num_heads, self.head_dim]).transpose(1, 2);

        let scores_cross = q_cross.matmul(&k_cross.transpose(-2, -1)) / (self.head_dim as f64).sqrt();
        let attn_weights_cross = scores_cross.softmax(-1, Kind::Float);
        
        let context_cross = attn_weights_cross.matmul(&v_cross).transpose(1, 2).contiguous().view([b, s, e]);
...
impl SRDecoderModel {
    pub fn new(vs: &nn::Path, vocab_size: i64, d_model: i64, nhead: i64, nhid: i64, nlayers: i64) -> Self {
        let alpha_context_encoder = AlphaConvEncoder::new(&vs.sub("alpha_context_encoder"), 64, d_model, 5);
        let target_token_emb = nn::embedding(&vs.sub("target_token_emb"), vocab_size, d_model, Default::default());
        ...
        for i in 0..nlayers {
            layers.push(TransformerDecoderLayer::new(&vs_layers.sub(format!("{}", i)), d_model, nhead, nhid));
        }
        
        let output_linear = nn::linear(&vs.sub("output_linear"), d_model, vocab_size, Default::default());
    ...
    pub fn forward(&self, shifted_target_tokens: &Tensor, alpha_matrix: &Tensor) -> (Tensor, Tensor) {
        let size = shifted_target_tokens.size();
        let _batch_size = size[0];
        let seq_len = size[1];
        let device = shifted_target_tokens.device();

        let memory_context = self.alpha_context_encoder.forward(alpha_matrix).unsqueeze(1);

        let tgt_vectors = self.target_token_emb.forward(shifted_target_tokens) * (self.d_model as f64).sqrt();
        ...
        let causal_mask = Tensor::ones([seq_len, seq_len], (Kind::Float, device))
            .triu(1)
            .masked_fill(&Tensor::ones([seq_len, seq_len], (Kind::Bool, device)).triu(1), std::f64::NEG_INFINITY);
