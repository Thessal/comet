...
pub static OP_TS_ARGMAX: OperatorSpec = OperatorSpec {
    ...
    execute: |args| match (&args[0], &args[1]) {
        (Signal::DataFrame(Some(a)), Signal::Int(Some(d))) => {
            ...
            let filled = reversed.nan_to_num(std::f64::NEG_INFINITY, std::f64::NEG_INFINITY, std::f64::NEG_INFINITY);
            let step_argmax = filled.argmax(-1, false).to_kind(a.kind());
            
            let all_nan = w.isnan().all_dim(-1, false);
            let nan = tch::Tensor::full(step_argmax.size().as_slice(), f64::NAN, (a.kind(), a.device()));
            let res = step_argmax.where_self(&all_nan.logical_not(), &nan);
