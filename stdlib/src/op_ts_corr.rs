    ...
    execute: |args| match (&args[0], &args[1], &args[2]) {
        (Signal::DataFrame(Some(x)), Signal::DataFrame(Some(y)), Signal::Int(Some(d))) => {
            ...
            for t in 0..t_len {
                ...
                let cov = (&diff_x * &diff_y).sum_dim_intlist(Some(&[0][..]), false, x.kind());
                let var_x = diff_x.square().sum_dim_intlist(Some(&[0][..]), false, x.kind());
                let var_y = diff_y.square().sum_dim_intlist(Some(&[0][..]), false, x.kind());
                ...
                let step_res = corr.where_self(&is_invalid.logical_not(), &nan);
...
mod tests {
    ...
    fn test_ts_corr() {
        ...
        let y = Tensor::from_slice(&[1.0, 2.0, 3.0, 4.0]).view([4, 1]);
        let out = (OP_TS_CORR.execute)(&[Signal::DataFrame(Some(x)), Signal::DataFrame(Some(y)), Signal::Int(Some(3))]);
        if let Signal::DataFrame(Some(res)) = out {
            let expected = Tensor::from_slice(&[f64::NAN, 1.0, 1.0, 1.0]).view([4, 1]);
