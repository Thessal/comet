...
pub static OP_TS_CORR: OperatorSpec = OperatorSpec {
    ...
    execute: |args| match (&args[0], &args[1], &args[2]) {
        (Signal::DataFrame(Some(x)), Signal::DataFrame(Some(y)), Signal::Int(Some(d))) => {
            ...
            let var_x = diff_x.square().sum_dim_intlist(Some(&[-1][..]), false, x.kind());
            let var_y = diff_y.square().sum_dim_intlist(Some(&[-1][..]), false, x.kind());
            ...
            let res = corr.where_self(&is_invalid.logical_not(), &nan);
