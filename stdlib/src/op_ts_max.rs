    ...
    execute: |args| match (&args[0], &args[1]) {
        (Signal::DataFrame(Some(a)), Signal::Int(Some(d))) => {
            ...
            for t in 0..t_len {
                ...
                let step_res = slice.nan_to_num(std::f64::NEG_INFINITY, std::f64::NEG_INFINITY, std::f64::NEG_INFINITY)
                    .amax(&[0], false);
                let is_neginf = step_res.eq(std::f64::NEG_INFINITY);
                let nan = tch::Tensor::full(step_res.size().as_slice(), f64::NAN, (a.kind(), a.device()));
                let step_res = step_res.where_self(&is_neginf.logical_not(), &nan);
...
mod tests {
    ...
    fn test_ts_max() {
        ...
        let out = (OP_TS_MAX.execute)(&[Signal::DataFrame(Some(a)), Signal::Int(Some(3))]);
        if let Signal::DataFrame(Some(res)) = out {
            let expected = Tensor::from_slice(&[1.0, 1.0, 3.0, 3.0, 5.0, 5.0]).view([6, 1]);
