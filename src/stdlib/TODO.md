...
| **UnaryOp** | `abs` | `signal (ptr)` | `out (ptr)` | |
| **Stateful UnaryOp** | `ts_delay`, `ts_diff`, `ts_mean`, `ts_sum`, `ts_decay_linear`, `ts_decay_exp`, `ts_std`, `ts_mae`, `ts_min`, `ts_max`, `ts_argmin`, `ts_argmax`, `ts_argminmax`, `ts_ffill` | `state (ptr)`, `signal (ptr)`, `period (i64)`, `len (i64)` | `out (ptr)` | `period` and `len` might be passed via `init` or `step`. |
