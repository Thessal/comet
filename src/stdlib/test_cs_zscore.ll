; src/stdlib/test_cs_zscore.ll

%CsZscoreState = type opaque
...
@.str.fmt = private unnamed_addr constant [76 x i8] c"Step %d: Input [%.1f, %.1f, %.1f, %.1f] -> Output [%.2f, %.2f, %.2f, %.2f]\0A\00", align 1

define i32 @main() {
entry:
    ; Initialize cs_zscore with period = 1, len = 4
    %state = call %CsZscoreState* @comet_cs_zscore_init(i64 1, i64 4)
    ...
    ; Step 1: [10.0, 30.0, 20.0, 20.0] -> mean=20, std=7.07
    ; Output: [-1.41, 1.41, 0, 0]
    ...
    store double 20.0, double* %a2
    %a3 = getelementptr inbounds double, double* %a_ptr, i64 3
    store double 20.0, double* %a3

    call void @comet_cs_zscore_step(%CsZscoreState* %state, double* %a_ptr, double* %out_ptr, i64 4)
    ...
    call i32 (i8*, ...) @printf(i8* %fmt_ptr, i32 1, double 10.0, double 30.0, double 20.0, double 20.0, double %o0_1, double %o1_1, double %o2_1, double %o3_1)
    
    ; Step 2: [NaN, 30.0, NaN, 10.0] -> mean=20, std=10.0
    ; Output: [NaN, 1.0, NaN, -1.0]
    ...
    ; Cleanup
    call void @comet_cs_zscore_free(%CsZscoreState* %state)
