use crate::comet::dag::{DagBuilder, DagOp};
use crate::comet::ast::Literal;
...
pub fn sequence_to_dag(seq: &[String]) -> Result<(DagBuilder, usize), String> {
    ...
    let root_id = parse_sequence(seq, &mut pos, &mut builder)?;
    if pos != seq.len() {
...
fn parse_sequence(seq: &[String], pos: &mut usize, builder: &mut DagBuilder) -> Result<usize, String> {
    if *pos >= seq.len() {
    ...
    if token.starts_with("CALL|") {
        ...
        for _ in 0..arity {
            ...
            *pos += 1;
            ...
            let child_id = parse_sequence(seq, pos, builder)?;
            args.push(child_id);
        }
...
mod tests {
    ...
    fn test_sequence_roundtrip_hash_consing() {
        ...
        if let DagOp::CallFn { args, .. } = &root_node.op {
            assert_eq!(args[0], args[1], "Hash consing failed to merge duplicate B arguments");
        } else {
