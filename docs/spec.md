...
-   **Types**: Concepts can be **Represented** by Concrete ADTs (`Series`, `DataFrame`) or **Type Synonyms** (`Volume`).
-   **Properties**: Concepts can be **Qualified** by **Type Classes** 
    -   **Semantic Constraints**: Marker Classes. (`NonZero`, `Stationary`).
    -   **Behaviors**: Type classes with Signatures. (`Comparator`, `Normalizer`).
...
## 5. Flows (Functions)
 ...
 Flows are functions, which can be defined with named arguments (uncurried).
 
 ```clean
 // Signature can be uncurried
 strategy(v1: Volume, v2: Volume) -> Signal
 strategy(v1, v2) = 
