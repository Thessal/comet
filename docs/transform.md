...
Mulitple trees are generated for each flow. Each tree are then passed to the Code Generator to emit the actual LLVM IR.
LLVM IRs are then linked to form a single library. To save disk space, LLVM IRs may be stored to the memory, as a binary form.